...
const SCORES = [
  {
    ...
    topBar: "bg-blue-500", range: "Range: 0 – 100",
  ...
  },
  {
    ...
    topBar: "bg-green-500", range: "Range: 0 – 100",
  ...
  },
  {
    ...
    topBar: "bg-red-500", range: "Range: −100 to +100",
  ...
  },
  {
    ...
    topBar: "bg-purple-500", range: "Unitless · typically −3 to +3",
    desc: "Is the current divergence unusual for this institution, relative to its own history?",
    how: "(Current divergence − historical mean) ÷ historical std over the last 30 readings. Normalises each institution against itself — not a universal baseline. |Z| > 2 triggers an alert.",
    chips: [
...
export default function Landing() {
  ...
  return (
    <div className="bg-gray-950 text-white overflow-x-hidden">
      ...
      {/* NAV */}
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-8 h-14 bg-gray-950/80 backdrop-blur-md border-b border-white/5">
      ...
      {/* HERO */}
      <section className="relative min-h-screen flex flex-col items-center justify-center px-8 pt-24 pb-16 overflow-hidden">
        {/* Grid background */}
        <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.03)_1px,transparent_1px)] bg-[size:40px_40px]" />
        {/* Glows */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-2/3 w-[600px] h-[600px] rounded-full bg-blue-600/10 blur-3xl pointer-events-none" />
        <div className="absolute top-2/3 left-1/3 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] rounded-full bg-emerald-600/7 blur-3xl pointer-events-none" />

        <div className="relative text-center max-w-3xl">
          <div className="animate-fade-up-1 inline-flex items-center gap-2 bg-blue-600/15 border border-blue-500/30 rounded-full px-4 py-1.5 text-xs text-blue-300 mb-8">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 live-dot" />
            Live · Updated every 6 hours
          </div>

          <h1 className="animate-fade-up-2 text-5xl md:text-6xl font-semibold leading-tight tracking-tight mb-6">
            What institutions <span className="text-blue-400">say</span><br />
            vs. what the public <span className="text-blue-400">shows</span>
          </h1>

          <p className="animate-fade-up-3 text-lg text-gray-400 leading-relaxed max-w-xl mx-auto mb-10">
          ...
          <div className="animate-fade-up-4 flex items-center justify-center gap-3">
            <Link href="/" className="bg-blue-600 hover:bg-blue-700 px-6 py-3 rounded-xl text-sm font-medium transition-all">
              Open dashboard
            </Link>
            <a href="#how" className="border border-white/20 hover:border-white/40 px-6 py-3 rounded-xl text-sm transition-all">
              How it works
            </a>
          </div>

          {/* Ticker */}
          <div className="animate-fade-up-5 mt-12 w-full max-w-3xl text-left">
            ...
            <div className="overflow-hidden rounded-xl border border-white/5">
              <div className="flex gap-3 p-3 ticker-inner">
                {allTicker.map((d, i) => (
                  <div key={i} className={`flex-shrink-0 rounded-lg px-4 py-3 min-w-[150px] border ${d.alert ? "border-red-500/40 bg-red-500/5" : "border-white/8 bg-white/3"}`}>
                    <p className="text-xs font-medium text-white mb-2">{d.name}</p>
      ...
      <div className="border-y border-white/5 bg-white/2">
        <div className="max-w-4xl mx-auto px-8 py-12 grid grid-cols-3 gap-8 text-center">
          {[
      ...
      <section id="how" className="py-24 px-8">
        <div className="max-w-4xl mx-auto">
          ...
          <FadeIn><h2 className="text-3xl font-semibold mb-3 tracking-tight">A two-sided signal engine</h2></FadeIn>
          <FadeIn><p className="text-gray-400 leading-relaxed max-w-lg mb-12">ICI ingests data from multiple sources, runs it through a custom NLP pipeline, and surfaces a single divergence score that tells you whether an institution's words match public behaviour.</p></FadeIn>

          <FadeIn>
            <div className="grid grid-cols-4 gap-0 rounded-xl overflow-hidden border border-white/8">
              {[
                { icon: "📄", step: "Official language", title: "SEC filings",    tags: ["10-K", "10-Q", "8-K", "Earnings calls"] },
                { icon: "🧠", step: "NLP analysis",      title: "FinBERT + hedging", tags: ["Tone detection", "25 hedge phrases", "VADER sentiment"] },
                ...
                <div key={p.step} className={`p-5 bg-white/3 ${i < 3 ? "border-r border-white/8" : ""}`}>
                  <div className="text-xl mb-3">{p.icon}</div>
                  <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">{p.step}</p>
                  <p className="text-sm font-medium text-white mb-3">{p.title}</p>
      ...
      {/* SCORES */}
      <section id="scores" className="py-24 px-8 bg-white/1 border-y border-white/5">
        <div className="max-w-4xl mx-auto">
          <FadeIn><p className="text-xs text-blue-400 uppercase tracking-widest mb-3">Score guide</p></FadeIn>
          <FadeIn><h2 className="text-3xl font-semibold mb-3 tracking-tight">What each number means</h2></FadeIn>
          <FadeIn><p className="text-gray-400 leading-relaxed max-w-lg mb-12">ICI produces four scores per institution per run. Here is exactly what they measure and how to read them.</p></FadeIn>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {SCORES.map((s, i) => (
              <FadeIn key={s.abbr} delay={i * 80}>
                <div className={`bg-white/3 border border-white/8 rounded-xl p-6 border-t-2 ${s.topBar.replace("bg-", "border-t-")}`}>
      ...
      {/* INSTITUTIONS */}
      <section id="institutions" className="py-24 px-8">
        <div className="max-w-4xl mx-auto">
          <FadeIn><p className="text-xs text-blue-400 uppercase tracking-widest mb-3">Coverage</p></FadeIn>
          <FadeIn><h2 className="text-3xl font-semibold mb-3 tracking-tight">Institutions currently tracked</h2></FadeIn>
          <FadeIn><p className="text-gray-400 leading-relaxed max-w-lg mb-12">Nine major institutions across banking, finance, government, and education — plus any orgs you add on the dashboard.</p></FadeIn>
      ...
      <section className="py-24 px-8 text-center border-t border-white/5">
        <FadeIn>
          <div className="max-w-xl mx-auto">
            <h2 className="text-3xl font-semibold mb-4 tracking-tight">Start tracking</h2>
            <p className="text-gray-400 leading-relaxed mb-8">Open the dashboard to explore all nine institutions, add your own, and run the pipeline on demand.</p>
